...
desktopDirPath = os.path.join(os.path.expanduser("~"), "Desktop")
...
class Instance(_Ctype):
    ...
    def __new__(cls, *args):
        ...
        if not args and plugin_path is not None:
            ...
            args = ['vlc', "--ignore-config", '--plugin-path=' + plugin_path]
        if PYTHON3:
...
class MediaPlayer(_Ctype):
    ...
    def take_snapshot(self, save_to=None, name=None, snap_format=".png", width=0, height=0):
        '''

        '''
        if not save_to:
            save_to = desktopDirPath
        if name:
            if snap_format:
                name += snap_format
            save_to = os.path.join(save_to, name)

        return libvlc_video_take_snapshot(self, 0, save_to, width, height)
